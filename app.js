...
function buildChecks({geo,bg,sharp,light,faceCount,out,spec}){
  ...
  if(geo){
    ...
    else if(hr>=0.28&&hr<=0.92){ hst="warn"; hco = hr<hmin
...
function makePhoto(src,sw,sh){
  ...
  if(g){
    cropH=(g.headRatio*sh)/mid; cropW=cropH*aspect; cx=g.centerX*sw;
    // Position vertically by the EYE LINE (the official method): place the eyes at the
    // spec's eye-height band so there's headroom above and the chin stays in frame.
    const e = spec.eyeFromBottom ? (spec.eyeFromBottom[0]+spec.eyeFromBottom[1])/2 : 0.58;
    const eyeY = sh*(1 - g.eyeFromBottom);            // eye line in source pixels
    cy = eyeY + cropH*(e-0.5);                         // => crop top = eyeY-(1-e)*cropH
  }
  else { cropH=Math.min(sh,sw/aspect); cropW=cropH*aspect; cx=sw/2; cy=sh/2; }
  // Never sample beyond the source frame: cap the crop to the video size (keeping the
  // output aspect ratio). Prevents the black band + horizontal squish when the head is
  // large in a smaller/portrait frame.
